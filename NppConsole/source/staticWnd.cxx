...
CStaticWnd::CStaticWnd() :
		m_hParWnd(NULL), m_hConWnd(NULL), m_hStdOut(INVALID_HANDLE_VALUE), m_conWndStyle(0), m_showFlag(SW_HIDE), m_ctrlCAction(CTRL_C_IGNORE), m_pGetCurrentConsoleFont(
		NULL), m_pGetConsoleFontSize(NULL){
...
LRESULT CALLBACK CStaticWnd::WinProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam){
	CStaticWnd *pWnd = (CStaticWnd*) ::GetWindowLongPtr(hwnd, GWL_USERDATA);
	switch(uMsg){
		case WM_NOTIFY: {
...
LRESULT WINAPI CStaticWnd::MouseProc(int nCode, WPARAM wParam, LPARAM lParam){
	...
	if(nCode == HC_ACTION && wParam == WM_LBUTTONUP){
		...
		CStaticWnd *pWnd = (CStaticWnd*) ::GetWindowLongPtr(s_hWnd, GWL_USERDATA);
...
BOOL CStaticWnd::CreateConsoleProcess(LPCTSTR cmd){
	...
	if(param){
		if(_tcslen(param) > 3){
			...
			if(paramEnd){
				...
				unsigned paramLen = _tcslen(param), paramEndLen = _tcslen(paramEnd);
	...
	m_param = paramTmp;
	return TRUE;
	CreateConsoleProcess_err: SLog("CreateConsoleProcess_err");
	return FALSE;
}
...
HWND CStaticWnd::Create(HWND hParWnd, LPCTSTR cmd, LPCTSTR matchLine){
	...
	::SetWindowLongPtr(s_hWnd, GWL_USERDATA, (LONG_PTR) this);
...
void CStaticWnd::Show(bool notifyPar){
	if(notifyPar)
		SendMessage(m_hParWnd, NPPM_DMMSHOW, 0, (LPARAM) s_hWnd);
...
void CStaticWnd::Hide(bool notifyPar){
	...
	if(notifyPar)
		::SendMessage(m_hParWnd, NPPM_DMMHIDE, 0, (LPARAM) s_hWnd);
...
void CStaticWnd::ProcessConsoleDBClick(UINT ptX, UINT ptY){
	...
	RECT conRect = { 0 };
	POINT clickPt = { (long)ptX, (long)ptY };
	IFV(!ScreenToClient(m_hConWnd, &clickPt));
	IFV(!GetClientRect(m_hConWnd, &conRect) || !PtInRect(&conRect, clickPt));SLog("clickPt.x: "<<clickPt.x<<" clickPt.y:"<<clickPt.y);
	SCROLLINFO hScrInfo = { 0 }, vScrInfo = { 0 };
	hScrInfo.cbSize = vScrInfo.cbSize = sizeof(SCROLLINFO);
	hScrInfo.fMask = vScrInfo.fMask = SIF_POS;
	IFV(!GetScrollInfo(m_hConWnd, SB_HORZ, &hScrInfo));SLog("hScrInfo.nPos "<<hScrInfo.nPos<<" hScrInfo.nPage:"<<hScrInfo.nPage);
	IFV(!GetScrollInfo(m_hConWnd, SB_VERT, &vScrInfo));SLog("vScrInfo.nPos "<<vScrInfo.nPos<<" vScrInfo.nPage:"<<vScrInfo.nPage);SLog("CONSOLE_FONT_INFO dwFontSize.X: "<<m_conFontSize.X<<" dwFontSize.Y: "<<m_conFontSize.Y);
	...
	coordBufCoord.X = 0;
	coordBufCoord.Y = 0;
	BOOL fReadSuccess = ReadConsoleOutput(m_hStdOut,      // screen buffer to read from
	...
	IFV(!conText.length());SLog("Clicked line length: "<<conText.length()<<" Clicked line: "<<conText.c_str());
	...
	ProcessConsoleDBClick_search: SLog("Search start: "<<pToSearchStart);SLog("Search end: "<<pToSearchEnd);
	for(int i = 0; i < nbFile; i++){
	...
	if(searchState == SEARCH_STEP_1){
		...
		ProcessConsoleDBClick_noLine: free(pattern);
	}
	ProcessConsoleDBClick_exit: for(int i = 0; i < nbFile; i++){
		delete fileNames[i];
	}
	delete[] fileNames;
}
...
void CStaticWnd::ProcessConsoleCtrlC(){
	SLogFu;SLog("m_ctrlCAction : "<<m_ctrlCAction);
