...
BOOL APIENTRY DllMain(HANDLE hModule, DWORD reasonForCall, LPVOID lpReserved){
	...
	switch(reasonForCall){
		...
		case DLL_PROCESS_DETACH:
			if(g_ToolBar.hToolbarBmp)
				::DeleteObject(g_ToolBar.hToolbarBmp);
			break;
...
INT_PTR CALLBACK DialogProc(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam){
	...
	_T("Build with mingw-w64 - Storozhenko Roman (d2.718l@gmail.com)\r\n");
		...
		case WM_COMMAND:
			switch(wParam){
				case IDC_BUTTON_APPLY: {
					...
					IFR(
							ERROR_SUCCESS!=RegCreateKeyEx(HKEY_CURRENT_USER, _T("Software\\NppConsole"), 0, NULL, REG_OPTION_NON_VOLATILE, KEY_ALL_ACCESS, NULL, &conKey, &dispos),
							TRUE);
					if(ERROR_SUCCESS == RegSetValueEx(conKey, _T("LinePattern"), 0, REG_SZ, (LPBYTE) cmd, _tcslen(cmd) * sizeof(TCHAR))){
						if(cc > 0)
							memcpy(g_savedLine, cmd, MAX_PATH * sizeof(TCHAR));
						else
							memset(g_savedLine, 0, MAX_PATH * sizeof(TCHAR));
					}
					...
					if(ERROR_SUCCESS == RegSetValueEx(conKey, NULL, 0, REG_SZ, (LPBYTE) cmd, _tcslen(cmd) * sizeof(TCHAR))){
						if(cc > 0)
							...
							memset(g_savedCmd, 0, MAX_PATH * sizeof(TCHAR));
					} SLog("g_savedCmd: "<<g_savedCmd); SLog("g_savedLine: "<<g_savedLine);
...
void AboutPlugin(){
	DialogBoxParam((HINSTANCE) g_hModule, MAKEINTRESOURCE(IDD_DIALOG_ABOUT), g_nppData._nppHandle, DialogProc, 0);
}
...
void beNotified(SCNotification *notifyCode){
	if(notifyCode->nmhdr.hwndFrom == g_nppData._nppHandle){
		if(notifyCode->nmhdr.code == NPPN_TBMODIFICATION){
			g_ToolBar.hToolbarBmp = (HBITMAP) ::LoadImage((HINSTANCE) g_hModule, MAKEINTRESOURCE(IDB_TLB_IMG), IMAGE_BITMAP, 0, 0,
